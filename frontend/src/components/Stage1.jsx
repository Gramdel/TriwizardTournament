...
export default function Stage1(props) {
    ...
    useEffect(() => {
        if (dragons.length === 0) {
            fetch('http://localhost/api/dragons/get').then((response) => response.json()).then((json) => {
                setDragons(json);
            });
        } else {
            let ids = [];
            dragons.forEach((dragon) => ids.push(dragon.vulnerability));
            fetch('http://localhost/api/vulnerabilities/get', {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        if (vulnerabilities.length > 0) {
            ...
            fetch('http://localhost/api/fighting_methods/get', {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
                            ...
                            <Box sx={{
                                ...
                                {results.length > 0 ? <Grid container spacing={1} mt={2}>
                                    ...
                                    <Grid item xs={2} sx={{
                                        ...
                                        <Box onClick={() => setShowResult(true)} sx={{
                                            width: 130,
                                            height: 130,
